...
hive -e "INSERT INTO TABLE temp.phone_wallpaper_summary SELECT p.TOTAL_PREVIEW, p.TOTAL_PREIVEW_FOR_PHONE, ( p.TOTAL_PREVIEW / p.TOTAL_PREIVEW_FOR_PHONE ) AS AVG_PREVIEW, f.FAVORITE, f.TOTAL_FAVORITE_FOR_PHONE, s.APPLY_ALL, s.PHONE_APPLY_ALL, sd.APPLY_DESKTOP, sd.PHONE_APPLY_DESKTOP, sl.APPLY_LOCK, sl.PHONE_APPLY_LOCK, sa.APPLY_ALL, sa.PHONE_APPLY_ALL, ro.RANDOM_ENABLE, ro.RANDOM_FOR_PHONE, rc.RANDOM_DISABLE, rc.RANDOM_DISABLE_PHONE, so.SHAKING_ENABLE, so.SHAKING_ENABLE_PHONE, sc.SHAKING_DISABLE, sc.SHAKING_DISABLE_PHONE, p.SUMMARY_DATE FROM temp.PHONE_PREVIEW_SUMMARY p INNER JOIN temp.PHONE_FAVORITES_SUMMARY f ON p.SUMMARY_DATE = f.SUMMARY_DATE INNER JOIN temp.PHONE_SETTING_ALL s ON p.SUMMARY_DATE = s.SUMMARY_DATE INNER JOIN temp.PHONE_SETTING_DESKTOP sd ON p.SUMMARY_DATE = sd.SUMMARY_DATE INNER JOIN temp.PHONE_SETTING_LOCK sl ON p.SUMMARY_DATE = sl.SUMMARY_DATE INNER JOIN temp.PHONE_SETTING_ALL sa ON p.SUMMARY_DATE = sa.SUMMARY_DATE INNER JOIN temp.PHONE_RANDOM_OPEN ro ON p.SUMMARY_DATE = ro.SUMMARY_DATE INNER JOIN temp.PHONE_RANDOM_CLOSE rc ON p.SUMMARY_DATE = rc.SUMMARY_DATE INNER JOIN temp.PHONE_SHAKING_OPEN so ON p.SUMMARY_DATE = so.SUMMARY_DATE INNER JOIN temp.PHONE_SHAKING_CLOSE sc ON p.SUMMARY_DATE = sc.SUMMARY_DATE WHERE p.SUMMARY_DATE = '$current' ; "