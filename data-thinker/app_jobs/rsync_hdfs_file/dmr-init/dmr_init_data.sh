...
do
  ...
  for tname in "${array[@]}"
    do
      ...
      hql="LOAD DATA INPATH 'hdfs://basic.internal.hadoop.10-148-10-80.scloud.letv.com:8020/user/cloudland/phone/v_newer/dmr.db/${tname}/dt=${current}/*' INTO TABLE dmr.${tname} PARTITION (dt=${current});"
      echo $hql
      hive -e "$hql"
  done
done
