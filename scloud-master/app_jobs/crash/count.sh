...
YESTERDAY2=`date -d '-1 days' +%Y%m%d`
  set_nostrit='set mapreduce.reduce.memory.mb=5120; set mapreduce.map.memory.mb=4096;set hive.exec.dynamic.partition=true; set hive.exec.dynamic.partition.mode=nonstrict; '
...
left join (select t1.build_version, t1.version_type,t1.dt,
      ...
      from (
        select imei_his_day.build_version, imei_his_day.dt as dt,
         ...
         where imei_his_day.dt=\"${YESTERDAY2}\" and sdk_phone_app_day.dt=\"${YESTERDAY2}\" and (imei_his_day.dt = sdk_phone_app_day.dt) 
